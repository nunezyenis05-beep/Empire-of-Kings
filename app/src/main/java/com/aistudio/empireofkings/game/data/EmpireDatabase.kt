...
@Database(
    entities = [
        ...
        CatalogAdminState::class
    ],
    // Schema 5 remains supported; catalog admin state is additive and local-only.
    version = 6,
    exportSchema = false
)
abstract class EmpireDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        val MIGRATION_5_6 = object : Migration(5, 6) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL(
                    """CREATE TABLE IF NOT EXISTS catalog_admin_state (
                        itemId TEXT NOT NULL,
                        availability TEXT NOT NULL,
                        publishedForDiamondSale INTEGER NOT NULL,
                        PRIMARY KEY(itemId)
                    )""".trimIndent()
                )
            }
