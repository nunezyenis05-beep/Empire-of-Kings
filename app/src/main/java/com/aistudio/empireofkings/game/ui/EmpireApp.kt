...
fun EmpireApp(
    ...
    val catalogAdminEntries by viewModel.catalogAdminEntries.collectAsStateWithLifecycle()
    ...
    when (currentScreen) {
        ...
        is ScreenRoute.SettingsAdmin -> {
            SettingsAndAdminScreen(
                ...
                catalogEntries = catalogAdminEntries,
                onAssignCatalogItems = { ids, result -> viewModel.assignCatalogItems(ids, result) },
                onPublishCatalogItems = { ids, result -> viewModel.publishCatalogItems(ids, result) },
                onRetireCatalogItems = { ids, result -> viewModel.retireCatalogItems(ids, result) },
